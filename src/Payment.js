    ...
    if(!error) {
        try {
            ...
            if(response.data.success) {
                axios.put("https://clear-slug-teddy.cyclic.app/addOrder" , { sTotal : Location.sTotal , discount : Location.discount , type : Location.type , id : Location.id , name : Location.name , mobile : Location.mobile , email : Location.email , address : Location.address, products : Location.products , pm : "ONLINE" , total : Location.total }).then(
                ()=>{
                    setLoading(false);
