...
function Search() {
    ...
    const Search = () => {
        setLoading(true);
        Axios.put("https://clear-slug-teddy.cyclic.app/getSearch" , {name : SearchThis}).then((response)=> {
    ...
    useEffect(()=>{
        setLoading(true);
        Axios.get("https://clear-slug-teddy.cyclic.app/getAllProducts").then((response) => {
            setProducts(response.data);
