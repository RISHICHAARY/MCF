...
function WishList(){
    ...
    const Delete = (id) => {
		...
		Axios.put("https://busy-lion-umbrella.cyclic.app/deleteWishList" , {id:Location.state.id , type : Location.state.type , file : OnCart}).then(()=>{
    ...
    useEffect(() => {
        ...
        Axios.put("https://busy-lion-umbrella.cyclic.app/getCart" , {type : Location.state.type , id:Location.state.id}).then((response)=>{
            setOnCart(response.data[0].wishlist);
            Axios.put("https://busy-lion-umbrella.cyclic.app/getSelectedProductss" , {id:response.data[0].wishlist}).then((response1) => {
