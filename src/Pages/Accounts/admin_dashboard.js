        ...
        Axios.get("https://bored-wasp-top-hat.cyclic.app/entireQueries").then((response)=>{
            Axios.get("https://bored-wasp-top-hat.cyclic.app/entireContactQueries").then((response1)=>{
                ...
                if(Received === undefined){
                    if(Received1 !== undefined && Received1.length !==0){
                        setQueryPercentage(String((parseFloat(parseFloat(Received1.length)/parseFloat(response1.data.length)))*100));
                    }
                }
                if(Received1 === undefined){
                ...
                else if(Received1.length===0){
                    setQueryPercentage(String((parseFloat(parseFloat(Received.length)/parseFloat(response.data.length)))*100));
                }
                else{
                setQueryPercentage(String((parseFloat(parseFloat(Received.length)+parseFloat(Received1.length)/parseFloat(response.data.length)+parseFloat(response1.data.length)))*100));
                }}setLoading(false);
                setOpen(false);})
        });
    }
                ...
                <>
                    <div className='row data'>
                        ...
                        <div className='col-5 personal-data'>
                            ...
                            <p>{UserData._id}</p>
                            <button className='add-buttons' onClick={()=>{Navigate("/AddProducts" , { state:{user_status: Location.state.status, user_name : Location.state.name , user:Location.state.user , type:Location.state.type , user_id:Location.state.id}})}}><i class="fa-solid fa-plus me-1"></i>ADD PRODUCT</button>
                        ...
                        <div className='in-progress-div-one'>
                            <CircularProgressbar value={parseInt(InProgress)} text={`${parseInt(InProgress)}%`} className='progress-text'
                        ...
                        <div className='in-progress-div-two'>
                            <CircularProgressbar value={parseInt(QueryPercentage)} text={`${parseInt(QueryPercentage)}%`} className='progress-text'
                                styles={buildStyles({pathTransitionDuration: 1000, trailColor: '#d2b7d8',textColor: 'white', backgroundColor: 'white', pathColor: '#ff9b9b'})} 
                            />
                            <p className='progress-text'>Queries Pending</p>
                        </div>
                    </div>
                    <div className='row offer'>
                                ...
                                <div className='opened-offer-div'>
                                    <div className='container'>
                                        <button className='close-button' onClick={()=>{setOpen(false)}}><i class="fi fi-br-cross"></i></button>
                                            ...
                                            <select className='add-offer-input-dd' onChange={(e)=>{setOfferType(e.target.value)}}>
                        ...
                        </div>
                        {
                            Offers.map((value)=>{
                                return(
                                    <div className='offer-div-valid' style={{backgroundColor:(value.method === "P")?"#a56eb0":"#5f2f87" , color:"white" , boxShadow:" 0 0 0.3em black"}}>
                    ...
                    <div className='row main-orders-div'>
                        <h1>QUERIES</h1>
                        {
                            (Queries.length === 0 && ContactQueries.length === 0)?
                            <p>No Queries</p>
                            :
                            Queries.map((value)=>{
                                return(
                                    <div className='container w-100 row order-row'>
                                        <p className='col-9 order-column order-id'>{value.question}</p>
                                        {
                                            (ExpandQueries === value._id)?
                                            <>
                                                <p className='col-3 order-column'>
                                                    <button className='expand-button'
                                                        onClick={()=>{setExpandQueries("")}}
                                                    >
                                                        <p className='button-text qurry'>HIDE</p>
                                                        <i class="fa-solid fa-chevron-up end-icon"></i>
                                                    </button>
                                                </p>
                                                {
                                                    ...
                                                    :
                                                    <p className='col-11 order-column'>{value.answer}</p>
                                                }
                                            </>
                                            :
                                            <p className='col-3 order-column'>
                                                ...
                                                >
                                                    <p className='button-text qurry'>EXPAND</p>
                                                    <i class="fa-solid fa-chevron-down end-icon"></i>
                                                </button>
                                            </p>
                                        }
