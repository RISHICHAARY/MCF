...
function Confirmation(){
    ...
    const HandleClick = () => {
        if(Validity){
            if(PaymentMode === "COD"){
                setLoading(true);
                Axios.put("https://busy-lion-umbrella.cyclic.app/addOrder" , { type : Location.state.type , sTotal : STotal , discount : Discount , id : Id , name : Name , mobile : Mobile , email : Email , address : DName+","+DMobile+","+House+", Street "+Street+","+City+","+State+","+Pin , products : OnCart , pm : "COD" , total : Total }).then(
                    (response)=>{
                        setLoading(false);
    ...
    const add = (rec , rec1) =>{
        ...
        Axios.get("https://busy-lion-umbrella.cyclic.app/getOffers").then((response3)=>{
