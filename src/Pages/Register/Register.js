...
function Products()
{
    ...
    useEffect(
        () =>{
            Axios.get("https://busy-lion-umbrella.cyclic.app/allUsers").then(
                (response) => {
                    setExistingUsers(response.data);
                }
            );
            Axios.get("https://busy-lion-umbrella.cyclic.app/allAdmins").then(
                (response) => {
                ...
                if(ExistingUsers.length === 0){
                    ...
                    for(var l =0 ; l < ExistingUsers.length ; l++){
                            if(Email.toString() === ExistingUsers[l].email.toString()){
                                Axios.post("https://busy-lion-umbrella.cyclic.app/DeleteUser" , {email:Email});
                                generator();
                                Navigate('/adminVerification' , 
                                    {   
                                        state:{name:Name , email:Email , mobile:Mobile , 
