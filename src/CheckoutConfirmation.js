...
function Confirmation(){
    ...
    const HandleClick = () => {
        ...
        if(PaymentMode === "COD" && AddressConfirmation === true && TCConfirmation === true){
            setLoading(true);
            Axios.put("https://magiccorner-b.onrender.com/addOrder" , { type : Location.state.type , sTotal : STotal , discount : Discount , id : Id , name : Name , mobile : Mobile , email : Email , address : House+", Street "+Street+","+City+","+State , products : OnCart , pm : "COD" , total : Total }).then(
                (response)=>{
                    setLoading(false);
    ...
    const add = (rec , rec1) =>{
        ...
        Axios.get("https://magiccorner-b.onrender.com/getOffers").then((response3)=>{
    ...
    useEffect(()=>{
            ...
            Axios.put("https://magiccorner-b.onrender.com/getCart" , {type : Location.state.type , id:Location.state.id}).then((response)=>{
                ...
                Axios.put("https://magiccorner-b.onrender.com/getSelectedProducts" , {id:response.data[0].on_cart}).then((response1) => {
